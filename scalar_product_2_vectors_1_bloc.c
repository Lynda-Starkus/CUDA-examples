/*Exercice 02 : un programme CUDA C qui permet produit scalaire de 2 vecteurs de taille N = 100 000, en mobilisant plusieurs threads de meme bloc*/
...
__global__ void kenrnel_scalar(int* vect1, int* vect2, int* result, int taille){
    ...
    temp[threadIdx.x] = vect1[threadIdx.x] + vect2[threadIdx.x];
...
int main(){
    ...
    //Lancer le Kernel
    kenrnel_add<<<1, THREADS_PER_BLOCK>>>(d_vect1, d_vect2, d_result, N);
